...
type mockKvStore struct {
	...
	ListenForInput  struct {
		Id       chan string
		Callback chan func(URL string, muxId uint64, add bool)
	}
}
...
func newMockKvStore() *mockKvStore {
	...
	m.ListenForInput.Callback = make(chan func(URL string, muxId uint64, add bool), 100)
	return m
}
func (m *mockKvStore) ListenFor(id string, callback func(URL string, muxId uint64, add bool)) {
...
type mockRegistry struct {
	GetListCalled chan bool
	GetListInput  struct {
		ID chan string
	}
	GetListOutput struct {
		Ret0 chan *datastructures.LinkedList
	}
}

func newMockRegistry() *mockRegistry {
	m := &mockRegistry{}
	m.GetListCalled = make(chan bool, 100)
	m.GetListInput.ID = make(chan string, 100)
	m.GetListOutput.Ret0 = make(chan *datastructures.LinkedList, 100)
	return m
}
func (m *mockRegistry) GetList(ID string) *datastructures.LinkedList {
	m.GetListCalled <- true
	m.GetListInput.ID <- ID
	return <-m.GetListOutput.Ret0
}
